...
func handleHTTPServer(ctx context.Context, u *url.URL, mktextrEndpoints *mktextr.Endpoints, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		mktextrServer *mktextrsvr.Server
	)
	{
		eh := errorHandler(ctx)
	...
	// Configure the mux.
	mktextrsvr.Mount(mux, mktextrServer)
	...
	for _, m := range mktextrServer.Mounts {
